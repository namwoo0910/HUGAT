...
def load_dataset():
    ...
    chk_hell = torch.load('../data/checkins/chk_hell.pt')
    geo_hell = torch.load('../data/landusage/geo_hell.pt')
    data = {}
    ...
    income = pd.read_pickle("../data/income/per_captia_income.pkl").sort_values(['zone']).dropna()
    income['UJAE001'] = income['UJAE001'] / 1000
    data['income'] = income
    crowded = pd.read_pickle("../data/income/popularity.pkl").sort_values(['zone']).dropna()
    data['popularity'] = crowded
    poverty = pd.read_pickle("../data/income/poverty.pkl").sort_values(['zone']).dropna()
    data['poverty'] = poverty
    cd = pd.read_pickle('../data/tracts/cd.pkl')
...
def load_heteograph_unified():
    g = load_graphs("../data/HUG/graph_2022_0602_chk_50_mob_75.bin")
    return g
